...
sites_map = ggplot()+
  ...
  geom_point(data = sites, aes(x=Y,y=X, fill = FishDiv, shape = Region), size = 5)+ #acutally plot the points of our sites
  scale_fill_gradient('Number of Fish Species', low = 'violet', high = 'midnightblue')+ #make the points fill properly
  scale_shape_manual(values = c(22,21,24)) + #use the shapes we want (need these ones cuz they have black borders)
  guides(shape = guide_legend(override.aes = list(size = 4, shape = c(0,1,2)))) + #display same shapes but no colours in the legend
  fte_theme_map_sites() +
